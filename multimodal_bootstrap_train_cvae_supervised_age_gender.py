...
def main(comb_label, hz_para_list, alpha_focal, gamma_focal, lambda_reg, dataset_name):
    ...
    for i_bootstrap in range(n_bootstrap):
        ...
        dataset_name = dataset_names = ['T1_volume', 'mean_T1_intensity', 'mean_FA', 'mean_MD', 'mean_L1', 'mean_L2', 'mean_L3', 'min_BOLD', '25_percentile_BOLD', '50_percentile_BOLD', '75_percentile_BOLD', 'max_BOLD']
        ...
        for dataset_name in dataset_names:
            ...
            age_bins_train, bin_edges = pd.cut(train_covariates['AGE'], 10, retbins=True, labels=bin_labels, duplicates='drop')  
            ...
            gender = train_covariates['PTGENDER'].values[:, np.newaxis].astype('float32') 
        ...
        for epoch in range(n_epochs): 
            ...
            for batch_idx, batch in enumerate(zip(*generator_train_list)):
                ...
                for modal in range(modalities):
                    model.optimizer1_list[modal] = torch.optim.Adam(list(model.encoder_list[modal].parameters()) + list(model.decoder_list[modal].parameters()), lr=clr)
                ...
                fwd_rtn = model.forward_multimodal(data_curr_list, cov_list)
                ...
                # loss_list = []
                # for modal in range(modalities):
                #     # loss = model.loss_function(data_curr, fwd_rtn_list[modal])
                #     loss = model.loss_function(data_curr_list[modal], fwd_rtn_list[modal])
                ...
                for modal in range(modalities):
                    epoch_ae_loss_avg.update(loss_list[modal]['ll'])
                    epoch_kl_loss_avg.update(loss_list[modal]['kl'])
                    epoch_total_loss_avg.update(loss_list[modal]['total'])


                # Zero out the gradients
                # model.optimizer1.zero_grad()
                for modal in range(modalities):
                    model.optimizer1_list[modal].zero_grad()
                # Backward pass through the model to calculate thecuda gradients
                # loss['total'].backward()
                for modal in range(modalities):
                    loss_list[modal]['total'].backward(retain_graph=True)
                # Update the weights using the gradients
                # model.optimizer1.step() 
                for modal in range(modalities):
                    model.optimizer1_list[modal].step()
                
                # Set the optimizer for the discriminator
                # model.optimizer2 = torch.optim.Adam(list(model.discriminator.parameters()), lr=clr)
                for modal in range(modalities):
                    model.optimizer2_list[modal] = torch.optim.Adam(list(model.discriminator_list[modal].parameters()), lr=clr)
                # # Get the current batch of data and move it to the appropriate device
                # data_curr = batch[0].to(DEVICE)
                # # Get the current batch of covariates and move it to the appropriate device
                # cov = batch[1].to(DEVICE)
                ...
                fwd_rtn2_list = []
                for modal in range(modalities):
                    # fwd_rtn2 = model.forward2(data_curr, cov, z_dim, modal)
                    fwd_rtn2 = model.forward2(data_curr_list[modal], cov_list[modal], z_dim, modal)
                    fwd_rtn2_list.append(fwd_rtn2)
                # Calculate the loss
                ...
                loss2_list = []
                for modal in range(modalities):
                    # loss2 = model.loss_function2(x = data_curr, fwd_rtn = fwd_rtn2_list[modal], alpha_focal=alpha_focal, gamma_focal=gamma_focal, lambda_reg=lambda_reg, logits=True, reduction='mean')
                    loss2 = model.loss_function2(x = data_curr_list[modal], fwd_rtn = fwd_rtn2_list[modal], alpha_focal=alpha_focal, gamma_focal=gamma_focal, lambda_reg=lambda_reg, logits=True, reduction='mean')
                ...
                for modal in range(modalities):
                    epoch_dc_loss_avg.update(loss2_list[modal]['dc_loss'])
                # Zero out the gradients
                for modal in range(modalities):
                    model.optimizer2_list[modal].zero_grad()
                # model.optimizer2.zero_grad()
                # Backward pass through the model to calculate the gradients
                for modal in range(modalities):
                    loss2_list[modal]['dc_loss'].backward()
                # loss2['dc_loss'].backward()
                # Update the weights using the gradients 
                for modal in range(modalities):
                    model.optimizer2_list[modal].step()
                # model.optimizer2.step() 
                    
                
                # Set the optimizer for the encoder
                for modal in range(modalities):
                    model.optimizer3_list[modal] = torch.optim.Adam(list(model.encoder_list[modal].parameters()), lr=clr)
                ...
                loss3_list = []
                for modal in range(modalities):
                    # fwd_rtn3 = model.forward3(data_curr, cov, modal)
                    fwd_rtn3 = model.forward3(data_curr_list[modal], cov_list[modal], modal)
                    fwd_rtn3_list.append(fwd_rtn3)
                    # loss3 = model.loss_function3(data_curr, fwd_rtn3)
                    loss3 = model.loss_function3(data_curr_list[modal], fwd_rtn3)
                    loss3_list.append(loss3)
                    epoch_gen_loss_avg.update(loss3['gen_loss'])
                    model.optimizer3_list[modal].zero_grad()
                    loss3['gen_loss'].backward()
                    model.optimizer3_list[modal].step() 
                ...
                for modal in range(modalities):
                    ...
                    loss = {}
                    loss_list.append(loss)
                    loss.update(loss_list[modal])
                    loss.update(loss2_list[modal])
                    loss.update(loss3_list[modal])
                    loss_list.append(loss)
...
if __name__ == "__main__":
    ...
    with open(result_focal_dir + '/result_multimodal.txt', 'a') as f:
        f.write('experiment settings\n')
        f.write('args.dataset: ' + str('Multimodal') + '\n')
        ...
        f.write('experiment done (under regularization loss)\n')
