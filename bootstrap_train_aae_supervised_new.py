...
def main(comb_label, hz_para_list, dataset_name):
    ...
    for i_bootstrap in range(n_bootstrap):
        ...
        ICV_bins_train, bin_edges = pd.qcut(dataset_df['PTEDUCAT'], q=3,  retbins=True, labels=bin_labels)
        ...
        def train_step(batch_x, batch_y,  pre, total):
            ...
            with tf.GradientTape() as ae_tape:
                ...
                #    dc_output_list[mck] = decoder(tf.concat([z, batch_y], axis=1), training=True)
                #decoder_output = sum(dc_output_list)/len(dc_output_list)
                #decoder_output_numpy = np.array(decoder_output)
                #mean_list[pre:total] += decoder_output_numpy
                #mean_list[i_bootstrap][]
                #variance = sum((dc_output_list - decoder_output)**2)/MCK
                #variance_numpy = np.array(variance)
                #variance_list[pre:total] += variance_numpy
                #tmp_noise_par = torch.FloatTensor(1, n_features).fill_(-3.0)
                #logvar_out = Parameter(data=tmp_noise_par, requires_grad=True)
                #output = Normal(loc=decoder_output, scale=logvar_out.exp().pow(0.5))
                #number = tf.reduce_mean(tf.reduce_mean(z_log_var, 0))
                #tmp_noise_par = tf.fill([1, n_features], number)
                #decoder_normal = tfd.Normal(loc=decoder_output, scale=tf.sqrt(tf.math.exp(tmp_noise_par)))
                #print(batch_x.shape, decoder_output.shape)
                # Autoencoder loss
                
                #a = batch_x.numpy()
                #a = torch.from_numpy(a)
                #ae_loss = calc_ll(a, decoder_normal)
        ...
        #tmp1 = np.mean(mean_list, axis=0)
        #mean_list_final[i_bootstrap] = tmp1
        #tmp2 = np.mean(variance_list, axis=0)
        #variance_list_final[i_bootstrap] = tmp2
        # Save models
    ...
    #mean_list_final = np.array(mean_list)
    #print(mean_list_final.shape)
    #mean_list_final = np.mean(mean_list, axis = 0)
    #np.save("mean_list_final.npy", mean_list_final)
    #variance_list_final = np.array(variance_list)
    #variance_list_final = np.mean(variance_list, axis = 0)   
    #np.save("variance_list_final.npy", variance_list_final)
