...
def test(args):
    ...
    HC_group = ids_df[ids_df['DIA'] == hc_label]
    MCI_group = ids_df[ids_df['DIA'] == mci_label]
    AD_group = ids_df[ids_df['DIA'] == ad_label]
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(HC_group)):
        ...
        # train_labels = np.ones(train_recon_errors.shape[0], dtype=np.float32)  # HC labels are 2
        train_labels = np.ones(train_recon_errors.shape[0], dtype=np.float32)  # HC labels are 2
...
if __name__ == "__main__":
    ...
    parser.add_argument('-K', '--n_splits', type=int, default=10, help='Number of splits for k-fold cross-validation.')
