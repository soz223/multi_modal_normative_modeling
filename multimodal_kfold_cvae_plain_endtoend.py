...
class cVAE_multimodal_endtoend(nn.Module):
    ...
    def loss_function_multimodal(self, xes, fwd_rtn, labels):
        ...
        for i in range(self.modalities):
            ...
            total = kl - recon + losses['classification_loss']
            losses['total'] += total
    ...
    def pred_recon(self, xes, c, DEVICE, combine):
        with torch.no_grad():
            ...
            return [x_recon.loc.cpu().detach().numpy() for x_recon in x_recons], diagnosis.cpu().detach().numpy()
...
def test(args):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(HC_group)):
        ...
        test_prediction_list, diagnosis = model.pred_recon(test_data_list, one_hot_covariates_test, DEVICE, combine)
