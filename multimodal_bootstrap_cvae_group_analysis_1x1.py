...
def main(hz_para_list, hc_label, disease_label, combine, dataset_resourse, procedure, epochs):
    ...
    for i_bootstrap in tqdm(range(n_bootstrap)):
        ...
        # roc_auc, tpr, accuracy, accuracy_in_hc, accuracy_in_ad, recall, specificity, significance_ratio = compute_classification_performance(reconstruction_error_df_averaged, clinical_df, disease_label, hc_label)
        ...
        # roc_auc, interp_tpr, accuracy, accuracy_in_hc, accuracy_in_ad, recall, specificity = compute_classification_performance(reconstruction_error_df_averaged, clinical_df, disease_label, hc_label) 
        auc_roc_list.append(roc_auc)
        accuracy_list.append(accuracy)
    ...
    #                        [np.percentile(effect_size_sig_df, 2.5, axis=0)],
    #                        [np.percentile(effect_size_sig_df, 97.5, axis=0)]))
    # # print('effect_size_sig_df.columns', effect_size_sig_df.columns)
    # # shape
    # # print('effect_size_sig_df.mean().values', effect_size_sig_df.mean().values.shape)
    # # print(save)
    # # np.savetxt("cvae_effect_size.csv", save, delimiter=",")
    # # save.to_csv("cvae_effect_size.csv", index=False)
    # # save to csv file
    # # Assuming effect_size_sig_df contains the data you want to save

    # # Convert the data to a NumPy array with a consistent data type (e.g., float64)
    # data_to_save = np.array([effect_size_sig_df.columns,
    #                         effect_size_sig_df.mean().values,
    #                         np.percentile(effect_size_sig_df, 2.5, axis=0),
    #                         np.percentile(effect_size_sig_df, 97.5, axis=0)])

    # # Save the data with a format specifier for floating-point numbers
    # np.savetxt("cvae_effect_size.csv", data_to_save, delimiter=",", fmt='%s')
...
if __name__ == "__main__":
    ...
    # for hc_label, disease_label in hc_patient_comb_list:
    #     main(dataset_name, hz_para_list, hc_label, disease_label, combine)
    ...
    with open(os.path.join(result_baseline, 'result_4.txt'), 'a') as f:
        ...
        # f.write('hz_para_list: ' + str(hz_para_list) + '\n')
        # f.write('\n\n\n')
        # f.write('Experiment settings: CVAE. Multimodal Average Report'SE-'+('MoE' if args.combine == 'moe' else 'PoE')\n')
        # f.write('Experiment settings: CVAE. {}. Dataset {}\n'.format('HC_vs_AD', 'SE-'+('MoE' if args.combine == 'moe' else 'PoE')))
        # f.write('Experiment settings: CVAE. {}. Dataset {} Procedure {}\n'.format('HC_vs_AD', 'SE-'+('MoE' if args.combine == 'moe' else 'PoE'), args.procedure))
